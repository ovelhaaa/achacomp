...
def generate_report(latest_path: Path = LATEST_JSON, output_path: Path | None = None, events_path: Path = EVENTS_JSON, summary_path: Path = SUMMARY_JSON) -> Path:
...
<table border='1'><thead><tr><th>evento</th><th>termo</th><th>loja</th><th>título</th><th>preço anterior</th><th>preço atual</th><th>variação</th><th>1a vez</th><th>última vez</th><th>vezes</th></tr></thead><tbody id='tb'></tbody></table>
<script>const DATA={json.dumps(data,ensure_ascii=False).replace('</','<\\/')};function esc(v){{return String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;')}}
function render(){{const e=document.getElementById('event').value;const only=document.getElementById('onlyNew').checked;const rows=DATA.filter(r=>(!e||r.event===e)&&(!only||r.is_new));document.getElementById('tb').innerHTML=rows.map(r=>{{const link=(r.link||'').trim(); const isSafe=/^(https?:\/\/|\/)/i.test(link); const linkCell=(link&&isSafe)?`<a href='${'{'}esc(link){'}'}'>abrir</a>`:(link?esc(link):''); return `<tr><td>${'{'}esc(r.event){'}'}</td><td>${'{'}esc(r.term){'}'}</td><td>${'{'}esc(r.store){'}'}</td><td>${'{'}esc(r.title){'}'}</td><td>${'{'}esc(r.previous_price_value){'}'}</td><td>${'{'}esc(r.last_price_value ?? r.price){'}'}</td><td>${'{'}esc(r.price_delta_percent){'}'}%</td><td>${'{'}esc(r.first_seen){'}'}</td><td>${'{'}esc(r.last_seen){'}'}</td><td>${'{'}esc(r.times_seen){'}'}</td></tr>`;}}).join('');}}
